...
public class PegawaiRestController {
    ...
    private PegawaiModel createPegawai(@Valid @RequestBody PegawaiModel pegawai, BindingResult bindingResult) {
        if (bindingResult.hasFieldErrors()) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "Request body invalid type or missing field."
            );
        } else {
    ...
    private Mono<PegawaiDetail> getUmur(@PathVariable("noPegawai") Long noPegawai) {
        try {
            Mono<PegawaiDetail> hasil = pegawaiRestService.getUmur(noPegawai);
            return hasil;
        } catch (NoSuchElementException e) {
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "Pegawai with ID " + String.valueOf(noPegawai) + " Not Found!"
